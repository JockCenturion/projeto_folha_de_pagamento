...
//import folha.eventos.Evento; //Criar pacote de evetos
import folha.Evento;
...
public abstract class Funcionario {
	...
	public void geraDemonstrativo() throws FolhaException {
		...
		try {
			d.incluiCredito("Salario base", this.salarioBase); //Inclui um lancamento
		} catch (Exception ex) {
	...
	private boolean eventoComum(Evento e) {
		if (e.getTipoEvento().equals("folha.EventoReajuste")) {
			dtSalario = e.getDtEvento();
			salarioBase = e.getValorEvento();
			return true;
		} else if (e.getTipoEvento().equals("folha.EventoRescisao")) {
